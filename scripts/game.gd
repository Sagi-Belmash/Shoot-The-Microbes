...
func _spawn_enemy():
	...
	enemy.linear_velocity = ENEMY_BASE_VELOCITY.rotated(direction)
	
	enemy.connect("destroyed", _on_enemy_destroyed)
	add_child(enemy)
	enemy.linear_velocity *= enemy.speed_scale[enemy.enemy_size]


func _on_enemy_destroyed(position: Vector2, enemy_size: int):
	if enemy_size > 0:
		if enemy_size == 2:
			score += 20
			...
			score += 10
		for i in range(3):
			call_deferred("_create_enemy_child", position, enemy_size - 1)
	else:
		score += 5
	$HUD.update_score(score)


func _create_enemy_child(position: Vector2, enemy_size: int):
	var enemy = enemy_scene.instantiate()
	enemy.position = position
	enemy.linear_velocity = ENEMY_BASE_VELOCITY.rotated(randf_range(0, 2 * PI))
	enemy.connect("destroyed", _on_enemy_destroyed)
	add_child(enemy)
	enemy.set_enemy_size(enemy_size)
	enemy.linear_velocity *= enemy.speed_scale[enemy_size]
